...
`Pointer<T, Unique>` that are known to obey Rust’s pointer provenance rules.
This allows you to confidently use `Pointer` in data structures rather than
other types like `NonNull` or raw pointers, which have variance and provenance
concerns.
...
If you need `NonNull` properties, use [`NonNullPointer`].
