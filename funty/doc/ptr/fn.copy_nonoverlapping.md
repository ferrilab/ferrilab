...
- [`core::ptr::copy_nonoverlapping`] &rArr;
  [`funty::ptr::copy_nonoverlapping`](crate::ptr::copy_nonoverlapping)
- [`<*T>::copy_to_nonoverlapping`][core_copyto] &rArr;
  [`Pointer::copy_to_nonoverlapping`]
- [`<*mut T>::copy_from_nonoverlapping`][core_copyfrom] &rArr;
...
Manually implement [`Vec::append`](std::vec::Vec::append):
...
fn append<T>(dst: &mut Vec<T>, src: &mut Vec<T>) {
  ...
  unsafe {
    ...
    let dst_ptr = dst.as_mut_ptr().wrap_funty().add(dst_len);
    let src_ptr = src.as_ptr().wrap_funty();
